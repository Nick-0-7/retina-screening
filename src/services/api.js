 ...
 * Supports dynamic environment detection, local/cloud fallback, and robust error diagnostics.
 */

const CLOUD_FALLBACK_URL = 'https://retina-screening.vercel.app/api/predict';

/**
 * Returns prioritized list of candidate endpoints based on execution context
 */
export function getCandidateEndpoints() {
  ...
  // Deployed / Production (Vercel, custom domain)
  return [
    '/api/predict',
    '/predict',
    CLOUD_FALLBACK_URL
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // If the server rejected it due to validation guard (e.g. 400 Bad Request),
        // do not fall back to other endpoints; this is an authentic model validation response.
        if (response.status === 400 || response.status === 422) {
          throw new Error(errorMessage);
        }

        // For other HTTP errors (e.g. 502/503/504), consider trying next candidate
        throw new Error(errorMessage);
      }
      ...
      // If it's a specific validation rejection from the backend, rethrow immediately
      if (err.message && (
      ...
      lastError = err;
      // Continue to next candidate endpoint in the loop
    }
  }
